// soft-rounded + palm: same warm five strokes, plus a wide round-capped pill
// behind the finger bases so the hand reads as one connected object. The
// palm is a sixth pseudo-finger drawn FIRST (behind); adopting it in the
// widget means adding 'palm' to FINGER_ORDER + a tint.
window.STYLES.push({
	name: 'soft-rounded-palm',
	blurb: 'soft-rounded with a palm — a wide pill behind the finger bases connects the strokes into one hand; the fist gets a real body in the pointer pose.',
	lineCap: 'round',
	poses: {
		open: {
			refSize: 855,
			fingers: {
				palm: { tipX: -85, tipY: 148, baseX: 158, baseY: 182, pipX: 36, pipY: 165, rectWidth: 195, rectBezOrCircle: 0, rectOrBez: 0 },
		...
		pointer: {
			refSize: 841,
			fingers: {
				palm: { tipX: -80, tipY: 68, baseX: 62, baseY: 96, pipX: -9, pipY: 82, rectWidth: 165, rectBezOrCircle: 0, rectOrBez: 0 },
