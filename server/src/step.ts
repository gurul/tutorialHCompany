...
function systemPrompt(question: string): string {
	const prompt = [
		...
		'- Use "answer" when the question is fully addressed.',
		'Fields:',
		'- "instruction": ONE short imperative sentence to the user, spoken aloud (e.g. "Click the New Invoice button, top right.").',
		'- "content" (answer): ONE short sentence, max 15 words. The user just watched every step, so do NOT recap them — confirm the outcome and stop (e.g. "That\'s it — your invoice is created.").',
